...
describe('auth routes', () => {
  it('rejects an incorrect password', async () => {
    const res = await loginPOST(jsonRequest('http://localhost/api/auth/login', { email: 'student@gurukul.dev', password: 'wrong' }));
  ...
  it('logs in with correct credentials and sets a session cookie', async () => {
    const res = await loginPOST(jsonRequest('http://localhost/api/auth/login', { email: 'student@gurukul.dev', password: 'student123' }));
    const body = await res.json();
    expect(res.status).toBe(200);
    expect(body.user.email).toBe('student@gurukul.dev');
  ...
  it('GET /me returns the user when a valid session cookie is sent', async () => {
    const loginRes = await loginPOST(jsonRequest('http://localhost/api/auth/login', { email: 'student@gurukul.dev', password: 'student123' }));
